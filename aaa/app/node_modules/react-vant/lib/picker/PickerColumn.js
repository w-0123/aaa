...
const PickerColumn = (0, _react().memo)((0, _react().forwardRef)((props, ref) => {
  ...
  const itemHeight = (0, _react().useMemo)(() => (0, _utils.unitToPx)(_itemHeight), [_itemHeight]);
  ...
  const getIndexByOffset = offset => (0, _utils.range)(Math.round(-offset / itemHeight), 0, options.length - 1);
  ...
  const onTouchMove = event => {
    ...
    const offset = (0, _utils.range)(startOffset.current + touch.deltaY.current, -(options.length * itemHeight), itemHeight);
    updateState({
      offset
    });
    const now = Date.now();
