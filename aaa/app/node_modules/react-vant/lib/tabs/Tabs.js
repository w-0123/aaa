...
const Tabs = (0, _react().forwardRef)((p, ref) => {
  ...
  const [index, setIndex] = (0, _useRefState.default)(defaultIndex);
  ...
  const offsetTopPx = (0, _react().useMemo)(() => (0, _utils.unitToPx)(props.offsetTop), [props.offsetTop]);
  // 下划线偏移量
  const [lineTranslateLeft, setLineTranslateLeft] = (0, _react().useState)(0);
  const [showLine, setShowLine] = (0, _react().useState)(false);
  ...
  const lineStyle = (0, _react().useMemo)(() => {
    ...
    return measureStyle;
  }, [color, props.lineHeight, props.lineWidth, lineTranslateLeft, immediateRef.current, showLine]);
  ...
  const scrollToCurrentContent = current => {
    ...
    if (contentTarget && scroller) {
      const immediate = (0, _utils.isObject)(props.scrollspy) ? (_b = (_a = props.scrollspy) === null || _a === void 0 ? void 0 : _a.scrollImmediate) !== null && _b !== void 0 ? _b : true : true;
